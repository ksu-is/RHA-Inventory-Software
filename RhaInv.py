...
root = Tk()
root.title("Python: JP Inventory System")
...
def Database():
    ...
    cursor.execute("SELECT * FROM `admin` WHERE `username` = 'rhadmin' AND `password` = 'rha1988'")
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `admin` (username, password) VALUES('rhaadmin', 'rha1988')")
        conn.commit()

def Exit():
    result = tkMessageBox.askquestion('JP Inventory System', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()

def Exit2():
    result = tkMessageBox.askquestion('JP Inventroy System', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        Home.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("JP Inventory System/Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def Home():
    ...
    Title.pack(pady=10)
    lbl_display = Label(Title, text="RHA Inventory System", font=('arial', 45))
    lbl_display.pack()
    menubar = Menu(Home)
...
def ViewForm():
    ...
    btn_reset = Button(LeftViewForm, text="Update", command=Reset)
...
def Delete():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('RHA Inventory System', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("JP Inventory System/View Product")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Logout():
    result = tkMessageBox.askquestion('JP Inventory System', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes': 
        admin_id = ""
        root.deiconify()
        Home.destroy()
...
#========================================LABEL WIDGET=====================================
lbl_display = Label(Title, text="JP Inventory System", font=('arial', 45))
lbl_display.pack()
